...
export const CreateCourseService = {
  ...
  async createSimpleCourse(data: CreateCourseRequest): Promise<CourseData> {
    try {
      ...
      const response = await axiosInstance.post<ApiResponse<CourseData>>('/courses/create-simple', requestData);
      
      // Kiểm tra response
      if (response.data && (response.data.statusCode === 201 || response.data.statusCode === 200) && response.data.data.success) {
