...
export default function ModuleList({ 
  ...
  const handleAddCurriculum = async (moduleId: string, type: CurriculumType) => {
    ...
    try {
      ...
      const newCurriculum = await CurriculumService.createCurriculum({
        title: type === 'LECTURE' ? 'Bài giảng mới' : 
               type === 'QUIZ' ? 'Bài kiểm tra mới' : 
               type === 'CODING_EXERCISE' ? 'Bài tập lập trình mới' :
               type === 'PRACTICE' ? 'Bài thực hành mới' : 'Bài tập mới',
        moduleId: moduleId,
        type: type,
      ...
      toast.success(`Đã thêm ${
        ...
        type === 'CODING_EXERCISE' ? 'bài tập lập trình' :
        type === 'PRACTICE' ? 'bài thực hành' : 'bài tập'
      } mới`);
    } catch (error: any) {
  ...
  const handleDeleteModule = async (moduleId: string) => {
    if (confirm('Bạn có chắc chắn muốn xóa module này không? Tất cả các curriculum trong module cũng sẽ bị xóa.')) {
      try {
        // Cập nhật UI trước
