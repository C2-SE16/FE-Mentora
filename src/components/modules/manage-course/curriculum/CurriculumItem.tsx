...
interface CurriculumItemProps {
  ...
  handleCurriculumTypeChange: (moduleId: string, curriculumId: string, newType: CurriculumType) => void;
...
export default function CurriculumItem({
  ...
  const hasContent = contentInfo && (
    (curriculum.type === 'LECTURE' && 
     'videoUrl' in contentInfo && 
     ('videoUrl' in contentInfo && contentInfo.videoUrl || 'articleContent' in contentInfo && contentInfo.articleContent)) ||
  ...
  return (
    ...
    >
      <div className="p-3 flex items-center">
        <div className="mr-2">
          {renderCurriculumTypeIcon(curriculum.type)}
        </div>
        ...
        <div className="flex-1">
          ...
          {contentInfo && (
            <div className="text-xs text-gray-500 mt-1">
              {contentInfo && curriculum.type === 'LECTURE' && 'videoUrl' in contentInfo && contentInfo.videoUrl && (
                <span className="flex items-center">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 mr-1" viewBox="0 0 20 20" fill="currentColor">
                    <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z" clipRule="evenodd" />
                  </svg>
                  Video {'duration' in contentInfo && contentInfo.duration ? `(${Math.floor(contentInfo.duration / 60)}:${String(contentInfo.duration % 60).padStart(2, '0')})` : ''}
                </span>
              )}
              {contentInfo && curriculum.type === 'LECTURE' && 'articleContent' in contentInfo && contentInfo.articleContent && (
                <span className="flex items-center">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 mr-1" viewBox="0 0 20 20" fill="currentColor">
                    <path fillRule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z" clipRule="evenodd" />
                  </svg>
              ...
              {contentInfo && curriculum.type === 'QUIZ' && (
                <span className="flex items-center">
                  ...
                  {'questions' in contentInfo && contentInfo.questions ? `${contentInfo.questions.length} câu hỏi` : 'Bài kiểm tra'}
                  {'timeLimit' in contentInfo && contentInfo.timeLimit ? ` (${contentInfo.timeLimit} phút)` : ''}
                </span>
              )}
              {contentInfo.isFree && (
                <span className="ml-2 bg-green-100 text-green-800 text-xs px-1 rounded">Miễn phí</span>
              )}
            </div>
          )}
        </div>
        ...
        <div className="px-3 pb-3 pt-0 border-t border-gray-100 mt-1">
          {!showDescriptionInput ? (
            <div className="flex justify-between items-center">
              ...
              >
                {contentInfo && 'description' in contentInfo && contentInfo.description ? 'Chỉnh sửa mô tả' : 'Thêm mô tả'}
              </button>
            </div>
          ) : (
