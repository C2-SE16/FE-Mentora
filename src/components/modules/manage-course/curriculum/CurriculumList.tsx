...
export default function CurriculumList({
  ...
  const handleCurriculumTitleChange = async (moduleId: string, curriculumId: string, newTitle: string) => {
    try {
      ...
      const updatedCurricula = curricula.map(curriculum => {
        if (curriculum.curriculumId === curriculumId) {
          ...
          if (updatedCurriculum.type === 'LECTURE' && updatedCurriculum.tbl_lectures && updatedCurriculum.tbl_lectures.length > 0) {
            updatedCurriculum.tbl_lectures[0].title = newTitle;
          } else if (updatedCurriculum.type === 'QUIZ' && updatedCurriculum.tbl_quizzes && updatedCurriculum.tbl_quizzes.length > 0) {
      ...
      if (curriculumToUpdate.type === 'LECTURE' && curriculumToUpdate.tbl_lectures && curriculumToUpdate.tbl_lectures.length > 0) {
        contentId = curriculumToUpdate.tbl_lectures[0].lectureId;
        await LectureService.updateLecture(contentId, { title: newTitle });
      } else if (curriculumToUpdate.type === 'QUIZ' && curriculumToUpdate.tbl_quizzes && curriculumToUpdate.tbl_quizzes.length > 0) {
  ...
  const handleCurriculumTypeChange = async (moduleId: string, curriculumId: string, newType: CurriculumType) => {
    try {
      // Cập nhật UI trước
      const updatedCurricula = curricula.map(curriculum => {
        if (curriculum.curriculumId === curriculumId) {
  ...
  const handleDeleteCurriculum = async (moduleId: string, curriculumId: string) => {
    if (confirm('Bạn có chắc chắn muốn xóa curriculum này không?')) {
      try {
        // Cập nhật UI trước
        const updatedCurricula = curricula.filter(curriculum => curriculum.curriculumId !== curriculumId);
    ...
    <div className="divide-y divide-gray-200">
      {curricula.length > 0 ? (
        ...
        <div className="p-4 text-center text-gray-500">
          Module này chưa có curriculum nào
        </div>
      )}
    </div>
  );
